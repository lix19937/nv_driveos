...
class Img_2D {
 public:
  ...
  bool Init(const char* filename) {
    ...
    if (ParseConfig(&config, (char*)filename) != 0) {
  ...
  bool InitV2(const char* filename) {
    memset(&config, 0, sizeof(config));
    if (ParseConfig(&config, (char*)filename) != 0) {
      LOG_ERR("ParseConfig failed");
      return false;
  ...
  bool DataFeed(const void* data[]) {
    for (int i = 0; i < MAX_SRC_LAYER_COUNT; ++i) {
      SrcLayer* layer = &config.srcLayers[i];
      if (!layer->isUsed) {
        continue;
      }
      if (ReadBufferFromBuffer(srcBufs[i].obj, data[i], config.fileIOMode) != 0) {
        LOG_ERR("DataFeed for src surf %d failed", i);
        return false;
      }
    }
  ...
  bool Run() {
    ...
    for (int i = 0; i < MAX_SRC_LAYER_COUNT; ++i) {
      ...
      if (layer->hasSrcRect || layer->hasDstRect || layer->hasTransform) {
        result = NvMedia2DSetSrcGeometry(
            handle,
            params,
            i,
            layer->hasSrcRect ? &layer->srcRect : NULL,
            layer->hasDstRect ? &layer->dstRect : NULL,
            layer->hasTransform ? layer->transform : NVMEDIA_2D_TRANSFORM_NONE);
        if (result != NVMEDIA_STATUS_OK) {
      ...
      if (layer->hasBlendMode) {
        result = NvMedia2DSetSrcBlendMode(handle, params, i, layer->blendMode, layer->constantAlpha);
        if (result != NVMEDIA_STATUS_OK) {
    ...
    void* dptr;
    NvSciBufObjGetCudaPtr(dstBuf, extMemBuffer, &dptr);
 ...
 private:
  bool IsSurfaceConfigured(const ConfigFileSurfaceAttrs* surf) {
    for (int i = 0; i < MAX_PLANE_COUNT; ++i) {
      if (surf->colorFmts[i].data[0] || surf->colorStds[i].data[0] || surf->widths[i] >= 0 || surf->heights[i] >= 0) {
        return true;
      }
    }
  ...
  bool IsRectConfigured(const ConfigFileRect* rect) {
    return rect->left >= 0 || rect->top >= 0 || rect->right >= 0 || rect->bottom >= 0;
  }

  bool IsLayerConfigured(const ConfigFileSrcLayer* layer) {
    return IsSurfaceConfigured(&layer->attrs) || IsRectConfigured(&layer->srcRect) ||
        IsRectConfigured(&layer->dstRect) || layer->filename.data[0] || layer->transform.data[0] ||
        layer->filter.data[0] || layer->blendMode.data[0] || layer->constAlpha >= 0;
  }

  int SetPlaneConfig(SurfaceAttrs* attrs, const ConfigFileSurfaceAttrs* conf, int plane, bool firstPlaneHasColorStd) {
    int retval = 0;
    ...
    if (strcmp(conf->colorFmts[plane].data, "U8V8") == 0) {
      ...
      LOG_ERR("Plane %d color format not %s", plane + 1, conf->colorFmts[plane].data[0] ? "valid" : "found");
      retval = 1;
    }
    ...
    if (firstPlaneHasColorStd) {
      if (strcmp(conf->colorStds[plane].data, "SRGB") == 0) {
        ...
        LOG_ERR("Plane %d color standard not %s", plane + 1, conf->colorStds[plane].data[0] ? "valid" : "found");
        retval = 1;
      }
  ...
  int SetSurfaceConfig(SurfaceAttrs* attrs, const ConfigFileSurfaceAttrs* conf) {
    int retval = 0;
    ...
    for (int i = 1; i < MAX_PLANE_COUNT; ++i) {
      if (conf->colorFmts[i].data[0] || conf->colorStds[i].data[0] || conf->widths[i] >= 0 || conf->heights[i] >= 0) {
        if (i != planeCount) {
  ...
  int SetRectConfig(NvMediaRect* rect, const ConfigFileRect* conf) {
    int retval = 0;
  ...
  int SetTransformConfig(NvMedia2DTransform* xform, const ConfigFileStr* conf) {
    int retval = 0;
  ...
  int SetFilterConfig(NvMedia2DFilter* filter, const ConfigFileStr* conf) {
    int retval = 0;
  ...
  int SetBlendConfig(SrcLayer* layer, const ConfigFileSrcLayer* conf) {
    int retval = 0;
  ...
  int SetFileIOModeConfig(FileIOMode* mode, const ConfigFileStr* conf) {
    int retval = 0;
  ...
  int SetConfig(Config* config, const ConfigFile* confFile) {
    int retval = 0;
  ...
  ConfigParamsMap ConfigParamStr(const char* name, ConfigFileStr* val) {
    ConfigParamsMap param = {
        name, val->data, TYPE_CHAR_ARR, 0, LIMITS_NONE, 0, 0, sizeof(val->data), NULL, SECTION_NONE};
    return param;
  }

  ConfigParamsMap ConfigParamFilename(const char* name, Filename* val) {
    ConfigParamsMap param = {
        name, val->data, TYPE_CHAR_ARR, 0, LIMITS_NONE, 0, 0, sizeof(val->data), NULL, SECTION_NONE};
    return param;
  }

  ConfigParamsMap ConfigParamInt(const char* name, int* val) {
    ConfigParamsMap param = {name, val, TYPE_INT, -1, LIMITS_NONE, 0, 0, 0, NULL, SECTION_NONE};
    return param;
  }

  ConfigParamsMap ConfigParamFloat(const char* name, float* val) {
    ConfigParamsMap param = {name, val, TYPE_FLOAT, -1, LIMITS_NONE, 0, 0, 0, NULL, SECTION_NONE};
    return param;
  }

  ConfigParamsMap ConfigParamSentinel(void) {
    ConfigParamsMap param = {NULL, NULL, TYPE_UINT, 0, LIMITS_NONE, 0, 0, 0, NULL, SECTION_NONE};
    return param;
  }

  int ParseConfig(Config* config, char* filename) {
    ConfigFile confFile;
  ...
  int AllocateSyncObj(SyncObj* sync, NvSciSyncModule syncModule, NvMedia2D* handle) {
    NvMediaStatus result = NVMEDIA_STATUS_OK;
    NvSciError sciResult = NvSciError_Success;
    ...
    sciResult = NvSciSyncAttrListSetAttrs(
        sync->attrListCpu, cpuAttrKvPairs, sizeof(cpuAttrKvPairs) / sizeof(cpuAttrKvPairs[0]));
    if (sciResult != NvSciError_Success) {
    ...
    sciResult = NvSciSyncAttrListReconcile(
        attrLists, sizeof(attrLists) / sizeof(attrLists[0]), &sync->attrListReconciled, &sync->attrListConflict);
    if (sciResult != NvSciError_Success) {
  ...
  void DeallocateSyncObj(SyncObj* sync) {
    if (sync->obj) {
  ...
  int AllocateBufObj(BufObj* buf, const SurfaceAttrs* attrs, NvSciBufModule bufModule, NvMedia2D* handle) {
    NvMediaStatus result = NVMEDIA_STATUS_OK;
    NvSciError sciResult = NvSciError_Success;
    ...
    // for cuda access
    CUuuid uuid;
    auto cuerr = cuDeviceGetUuid(&uuid, current_device);
    if (cuerr != 0) {
      printf("cuDeviceGetUuid error:%d\n", cuerr);
      exit(1);
    }

    // Creates a new, single slot, unreconciled NvSciBufAttrList associated with the input NvSciBufModule with empty
    // NvSciBufAttrKeys.
    /* Create the attribute list describing the surface */
    sciResult = NvSciBufAttrListCreate(bufModule /* not used */, &buf->attrList /* out  	NvSciBufAttrList *  */);
    if (sciResult != NvSciError_Success) {
    ...
    /* Fill NvMedia 2D mandatory attributes to the attribute list */
    result = NvMedia2DFillNvSciBufAttrList(handle, buf->attrList /* out */);
    if (result != NVMEDIA_STATUS_OK) {
    ...
    bool cpuAccess = true;
    NvSciBufAttrValAccessPerm perm = NvSciBufAccessPerm_ReadWrite; // lix add
    ...
    NvSciBufAttrKeyValuePair attrKvPairs[] = {
        ...
        {NvSciBufImageAttrKey_PlaneWidth, attrs->planeWidths, attrs->planeCount * sizeof(attrs->planeWidths[0])},
        {NvSciBufImageAttrKey_PlaneHeight, attrs->planeHeights, attrs->planeCount * sizeof(attrs->planeHeights[0])},
        {NvSciBufGeneralAttrKey_GpuId, &uuid, sizeof(uuid)}, // lix
    };
    sciResult = NvSciBufAttrListSetAttrs(buf->attrList, attrKvPairs, sizeof(attrKvPairs) / sizeof(attrKvPairs[0]));
    if (sciResult != NvSciError_Success) {
    ...
    sciResult = NvSciBufAttrListReconcile(
        &buf->attrList,
        1,
        &buf->attrListReconciled, /* out */
        &buf->attrListConflict); /* out */
    if (sciResult != NvSciError_Success) {
    ...
    sciResult = NvSciBufAttrListIsReconciled(buf->attrListReconciled /* in */, &isReconciled);
    if (sciResult != NvSciError_Success) {
      printf("NvSciBufAttrListIsReconciled failed with %d\n", sciResult);
      return 1;
    }
    if (!isReconciled) {
      printf("NvSciBufAttrListIsReconciled false \n");
      return 1;
    }
    //-----------------------
  ...
  void DeallocateBufObj(BufObj* buf) {
    if (buf->obj) {
  ...
  int AllocatePixelDataBuffer(PixelDataBuffer* px, NvSciBufObj sciBuf, FileIOMode mode) {
    ...
    if (mode == FILE_IO_MODE_NVSCI) {
      if (px->planeCount == 1 &&
          IsInterleavedYUV(((const NvSciBufAttrValColorFmt*)attrs[PLANE_COLORFMT_ATTR_IDX].value)[0])) {
        px->planeCount = 3;
  ...
  int AllocatePixelDataBufferStub(PixelDataBuffer* px, NvSciBufObj sciBuf, FileIOMode mode, const void* out_ptr) {
    NvSciBufAttrList attrList;
    NvSciError sciResult = NvSciBufObjGetAttrList(sciBuf, &attrList);
    if (sciResult != NvSciError_Success) {
      LOG_ERR("Failed to get buffer attribute list");
      return 1;
    }
    ...
    px->buffer = (void*)out_ptr; // malloc(px->size); //
  ...
  int ReadWritePixelDataLineByLine(NvSciBufObj sciBuf, PixelDataBuffer* px, bool write) {
    ...
    // printf("px->planeCount :%u  %s\n", px->planeCount, write ? "write" : "read");
    ...
    for (uint32_t plane = 0; plane < px->planeCount; ++plane) {
      ...
      // printf(">>> %10lu  %10u  %10u %10u\n", sciBufPlaneOffset, sciBufPlanePitch, planeHeight,
      // px->planePitches[plane]);
      ...
      for (uint32_t line = 0; line < planeHeight; ++line) {
        ...
        if (write) { // write data to scibuf
          memcpy(sciBufLine, pxBufLine, px->planePitches[plane]);
        } else { // read data from scibuf
  ...
  int ReadBufferFromFile(NvSciBufObj buffer, Filename* filename, FileIOMode mode) {
    // printf("\n>>> ReadBufferFromFile mode:%s\n", mode == FILE_IO_MODE_NVSCI ? "nvsci" : "linebyline");
    ...
    switch (mode) {
      case FILE_IO_MODE_NVSCI:
        sciResult = NvSciBufObjPutPixels(buffer, NULL, (const void**)px.planePtrs, px.planeSizes, px.planePitches);
        if (sciResult != NvSciError_Success) {
  ...
  int ReadBufferFromBuffer(NvSciBufObj& buffer, const void* data, FileIOMode mode) {
    NvSciError sciResult = NvSciError_Success;
    PixelDataBuffer px;
    memset(&px, 0, sizeof(px));
    if (AllocatePixelDataBufferStub(&px, buffer, mode, (void*)data) != 0) {
      LOG_ERR("Failed to allocate pixel data buffer");
      return 1;
    }
    ...
    switch (mode) {
      case FILE_IO_MODE_NVSCI:
        sciResult = NvSciBufObjPutPixels(buffer, NULL, (const void**)px.planePtrs, px.planeSizes, px.planePitches);
        if (sciResult != NvSciError_Success) {
          LOG_ERR("Failed to write data to scibuffer using NvSci");
          return 1;
        }
        break;
      ...
      case FILE_IO_MODE_LINE_BY_LINE:
        if (ReadWritePixelDataLineByLine(buffer, &px, true) != 0) {
          LOG_ERR("Failed to write data to scibuffer line-by-line");
          return 1;
        }
        break;
    }
  ...
  int WriteBufferToFile(NvSciBufObj buffer, Filename* filename, FileIOMode mode) {
    // printf("\n>>> WriteBufferToFile mode:%s\n", mode == FILE_IO_MODE_NVSCI ? "nvsci" : "linebyline");
  ...
  int NvSciBufObjGetCudaPtr(BufObj& buf, cudaExternalMemory_t& extMemBuffer, void** dptr) {
    ...
    bool isReconciled = false;
    NvSciError sciResult = NvSciBufAttrListIsReconciled(buf.attrListReconciled /* in */, &isReconciled);
    if (sciResult != NvSciError_Success) {
      printf("NvSciBufAttrListIsReconciled failed with %d\n", sciResult);
      return 1;
    }
    ...
    sciResult = NvSciBufAttrListGetAttrs(
        buf.attrListReconciled /* in */, bufattrs /* in/out */, sizeof(bufattrs) / sizeof(NvSciBufAttrKeyValuePair));
    if (sciResult != NvSciError_Success) {
      printf("Failed to get buffer attributes, NvSciBufAttrListGetAttrs error 0x%x\n", sciResult);
      return 1;
    }

    auto size = *(static_cast<const uint64_t*>(bufattrs[0].value));
    auto aligned_size = *(static_cast<const uint64_t*>(bufattrs[1].value));

    auto plane_w = *(static_cast<const uint32_t*>(bufattrs[3].value));
    auto plane_h = *(static_cast<const uint32_t*>(bufattrs[4].value));
    auto plane_pitch = *(static_cast<const uint32_t*>(bufattrs[5].value));
    auto bpp = *(static_cast<const uint32_t*>(bufattrs[6].value));
    auto plane_h_aligned = *(static_cast<const uint32_t*>(bufattrs[7].value));

    auto lp = *(static_cast<const uint64_t*>(bufattrs[8].value));
    auto rp = *(static_cast<const uint64_t*>(bufattrs[9].value));
    auto ali = *(static_cast<const uint64_t*>(bufattrs[10].value));
    auto layout = *(static_cast<const NvSciBufAttrValImageLayoutType*>(bufattrs[11].value));

    auto gpuid = *(static_cast<const NvSciRmGpuId*>(bufattrs[12].value));
    ...
    auto cudaErr = cudaImportExternalMemory(&extMemBuffer, &memHandleDesc);
    if (cudaSuccess != cudaErr) {
      printf(
          "Unable to import buffer to CUDA error:%d %s \n"
          "size:%ld \n"
          "aligned_size:%ld \n"
          "plane_w:%d \n"
          "plane_h:%d \n"
    ...
    cudaErr = cudaExternalMemoryGetMappedBuffer(dptr, extMemBuffer, &bufferDesc);
    if (cudaSuccess != cudaErr) {
      printf("Unable to map CUDA buffer \n");
      return 1;
    }

    size_t m_hostBufLen = plane_h_aligned * plane_pitch; // 1024*672*4
    std::vector<uint8_t> m_pHostBuf(m_hostBufLen);
    cudaErr = cudaMemcpy(m_pHostBuf.data(), *dptr, m_hostBufLen, cudaMemcpyDeviceToHost);
    if (cudaSuccess != cudaErr) {
      printf("Unable to cudaMemcpy %s\n", cudaGetErrorString(cudaErr));
      return 1;
    }

    auto out_name = std::string(config.dstFilename.data) + "zz.bgra";
    auto m_pOutputFile = fopen(out_name.c_str(), "wb");
    if (m_hostBufLen > 0) {
      char* buffer = (char*)m_pHostBuf.data();
  ...
  int NvSciBufObjGetDstCpuPtr(BufObj buf, void** dptr) {
    NvSciError sciResult = NvSciBufObjGetCpuPtr(buf.obj, dptr);
    if (sciResult != NvSciError_Success) {
      printf("Failed to get buffer CPU pointer\n");
      return 1;
    }
    return 0;
  }
...
inline int64_t GetUs() {
  ...
  int64_t msecs_time = (time_now.tv_sec * 1000 * 1000) + (time_now.tv_usec); // us
  return msecs_time;
}
...
int batch() {
  const int BATCH = 6;
  Img_2D img2d[BATCH];
  const char* names[BATCH]{"0.cfg", "3.cfg", "8.cfg", "9.cfg", "10.cfg", "11.cfg"};

  for (int idx = 0; idx < BATCH; ++idx) {
    auto str = (char*)names[idx];
    auto& runner = img2d[idx];

    if (!runner.Init(str)) {
      LOG_ERR("Init failed, configfile", str);
      return 1;
    }
  }
  ...
  for (int idx = 0; idx < BATCH; ++idx) {
    ...
    if (!runner.Run()) {
      LOG_ERR("Run failed, configfile", names[idx]);
      return 1;
    }
  }

  auto et = GetUs();
  float avg = (et - st) / 1000.f;

  printf("%s done avg time: %f ms\n", __FUNCTION__, avg);
  return 0;
}
...
int batch_by_thread() {
  ...
  thread_vec.reserve(BATCH);

  for (int idx = 0; idx < BATCH; ++idx) {
    thread_vec.emplace_back(std::thread(&Img_2D::Run, &img2d[idx]));
  }
...
int one_cfg() {
  const char* names{"all_in_one.cfg"};

  Img_2D img2d;
  if (!img2d.Init((char*)names)) {
    LOG_ERR("Init failed, configfile", names);
    return 1;
  }

  auto st = GetUs();
  if (!img2d.Run()) {
    LOG_ERR("Run failed, configfile", names);
    return 1;
  }
...
int one_0cfg() {
  const char* names{"0.cfg"}; // 1920*1080*2

  Img_2D img2d;
  if (!img2d.InitV2((char*)names)) {
    LOG_ERR("Init failed, configfile", names);
    return 1;
  }

  const int ele_num = 1920 * 1080 * 2;
  std::vector<uint8_t> img_data(ele_num);
  read_bin<uint8_t>("rear_mid_1920_1080_yuv2.bin", img_data.data(), ele_num /* ele num */);

  const void* inputs[]{(void*)img_data.data()};

  auto st = GetUs();

  if (!img2d.DataFeed(inputs)) {
    LOG_ERR("Init failed, configfile", names);
    return 1;
  }
...
int batch_by_threadV2() {
  const int BATCH = 6;
  Img_2D img2d[BATCH];
  const char* names[BATCH]{"0.cfg", "3.cfg", "8.cfg", "9.cfg", "10.cfg", "11.cfg"};
  const char* bin_names[BATCH]{
      "rear_mid_1920_1080_yuv2.bin",
      "front_mid_3840_2160_nv12.bin",
      "fish_front_1920_1440yuv2.bin",
      "fish_left_1920_1440yuv2.bin",
      "fish_front_1920_1440yuv2.bin",
      "fish_front_1920_1440yuv2.bin"};
  const int ele_num[BATCH]{
      1920 * 1080 * 2, int(3840 * 2160 * 1.5), 1920 * 1440 * 2, 1920 * 1440 * 2, 1920 * 1440 * 2, 1920 * 1440 * 2};

  for (int idx = 0; idx < BATCH; ++idx) {
    auto str = (char*)names[idx];
    auto& runner = img2d[idx];

    if (!runner.InitV2(str)) {
      LOG_ERR("Init failed, configfile", str);
      return 1;
    }
  }

  std::vector<uint8_t> img_data[BATCH];

  for (int idx = 0; idx < BATCH; ++idx) {
    auto& img = img_data[idx];
    img.resize(ele_num[idx]);
    read_bin<uint8_t>(bin_names[idx], img.data(), img.size() /* ele num */);
  }
  ...
  auto warp_run = [&img_data, &img2d](int idx) {
    const void* _inputs[]{(void*)(img_data[idx].data())};

    if (!img2d[idx].DataFeed(_inputs)) {
      LOG_ERR("Init failed, configfile");
      return;
    }

    if (!img2d[idx].Run()) {
      LOG_ERR("Run failed, configfile");
      return;
    }
  };

  for (int idx = 0; idx < BATCH; ++idx) {
    thread_vec.emplace_back(std::thread(warp_run, idx));
  }
...
/*
// /home/lix/drive_linux_sample/drive-linux/samples/nvmedia_6x/utils/config_parser.c \

#!/bin/bash 

# -D _FILE_DUMP_

/home/lix/drive_linux_sample/toolchains/aarch64--glibc--stable-2022.03-1/bin/aarch64-buildroot-linux-gnu-g++ \
-Os -O2 -fomit-frame-pointer -finline-functions -finline-limit=300 -fgcse-after-reload -fno-strict-aliasing    \
-Wall -mharden-sls=all -Wcast-align  \
image_2d.cpp        \
/home/lix/drive_linux_sample/drive-linux/samples/nvmedia_6x/utils/config_parser.c \
/home/lix/drive_linux_sample/drive-linux/samples/nvmedia_6x/utils/log_utils.c \
-I/home/lix/drive_linux_sample/drive-linux/samples/nvmedia_6x/utils  \
-I/home/lix/drive_linux_sample/drive-linux/include/nvmedia_6x \
-I/home/lix/drive_linux_sample/drive-linux/include            \
-I/home/lix/nv-tensorrt-repo-ubuntu2004-cuda11.4-trt8.5.10.4-x86-host-ga-20221229_1-1_amd64/dpkg-x/usr/local/cuda-11.4/targets/aarch64-linux/include/ \
-L/home/lix/drive_linux_sample/drive-linux/lib-target         \
-L/home/lix/drive_linux_sample/drive-linux/lib-target/x11     \
-L/home/lix/nv-tensorrt-repo-ubuntu2004-cuda11.4-trt8.5.10.4-x86-host-ga-20221229_1-1_amd64/dpkg-x/usr/local/cuda-11.4/targets/aarch64-linux/lib/ \
-Wl,-rpath-link=/home/lix/drive_linux_sample/drive-linux/lib-target \
-Wl,-rpath-link=/home/lix/drive_linux_sample/drive-linux/lib-target/x11 \
-Wl,--dynamic-linker=/lib/ld-linux-aarch64.so.1 -D _FILE_DUMP_ \
-lnvmedia2d -lnvscibuf -lnvscisync  -lcuda -lcudart -lpthread -o t_image_2d


mv  t_image_2d parking/

# scp parking/t_image_2d    nvidia@192.168.8.107:/data/ljw/parking/
*/
